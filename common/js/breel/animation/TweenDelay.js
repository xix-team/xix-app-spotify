(function() {
	...
	if(!namespace.TweenDelay) {
		...
		p._init = function() {
			...
			this._updateTween = new (TWEEN.Tween)({"update": 0}).onUpdate(this._updateCallback);
		}
		...
		p.animateTo = function(aTo, aTime, aEasingFunction, aDelay) {
			...
			if(aDelay == 0) {
				...
				this._delayedTweens.push({"startTime": startTime, "to": aTo, "time": aTime, "easingFunction": aEasingFunction});
				this._updateTween.to({"update": 1}, 1000*aDelay+100).start();
			}

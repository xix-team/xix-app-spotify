...
(function() {
	if(bongiovi.GLModel === undefined) {
		...
		p.updateNumVertex = function(numVertex) {
			this._numVertex = numVertex;
			this._createIndexBuffer();
		};
		...
		p.clearBuffers = function() {
			for(var i=0; i<this._vertices.length; i++) this._vertices[i] = 0;
			for(var i=0; i<this._uvs.length; i++) this._uvs[i] = 0;
			// for(var i=0; i<this._attributes.length; i++) {
			// 	var atr = this._attributes[i].attributes;
			// 	for(var j=0; j<atr.length; j++) console.log( atr[j] );
			// }
				...
				if(this._uniforms[o.name] == undefined || this._uniforms[o.name]!=o.value) {
					if(o.type.indexOf("Matrix") == -1) this.gl[o.type](shaderProgram[o.name], o.value);
						...
						// try {
							this.gl.uniformMatrix3fv(shaderProgram[o.name], false, o.value);
						// } catch(e) {
						// 	console.log( "Fail set uniform : ", o );
						// }
		...
		p.generateBuffer = function() {
			// console.debug( "Generate Buffer" );
			this._arrayVertices = new Float32Array(this._vertices);
			this._arrayUvs      = new Float32Array(this._uvs);

			this.vBufferPos = this.gl.createBuffer();
			this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.vBufferPos);
			this.gl.bufferData(this.gl.ARRAY_BUFFER, this._arrayVertices, this.gl.DYNAMIC_DRAW);
			this.vBufferPos.itemSize = 3;
			// this.vBufferPos.numItems = this._numVertex;

			this.vBufferUV = this.gl.createBuffer();
			this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.vBufferUV);
			this.gl.bufferData(this.gl.ARRAY_BUFFER, this._arrayUvs, this.gl.DYNAMIC_DRAW);
			...
			for ( var i=0; i<this._attributes.length; i++) {
				...
				var arrayAttr = new Float32Array(this._attributes[i].attributes);
				this._arrayAttributes.push(arrayAttr)

				this.gl.bufferData(this.gl.ARRAY_BUFFER, arrayAttr, this.gl.DYNAMIC_DRAW);	
			}
		}


		p.updateBuffer = function() {
			// console.debug( "Buffer Update" );
			updateFloat32Array(this._arrayVertices, this._vertices);
			updateFloat32Array(this._arrayUvs, this._uvs);
			...
			for ( var i=0; i<this._attributes.length; i++) {
				updateFloat32Array(this._arrayAttributes[i], this._attributes[i].attributes);
				this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this._attributes[i].buffer);
				this.gl.bufferSubData(this.gl.ARRAY_BUFFER, 0, this._arrayAttributes[i]);	
				// this.gl.bufferData(this.gl.ARRAY_BUFFER, this._arrayAttributes[i], this.gl.DYNAMIC_DRAW);			
			}
		};


		var updateFloat32Array = function(targetArray, buffer) {
		...
		p._createIndexBuffer = function() {
			...
			for ( var i=0; i<numQuad; i++) {
				this._indices.push(0+i*4, 1+i*4, 3+i*4, 1+i*4, 2+i*4, 3+i*4);
			}

			this.iBuffer = this.gl.createBuffer();
			this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, this.iBuffer);
			this.gl.bufferData(this.gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(this._indices), this.gl.DYNAMIC_DRAW);
			this.iBuffer.itemSize = 1;
			this.iBuffer.numItems = this._indices.length;
		}

		p.createIndexBuffer = function(numVertex) {
			this._numVertex = numVertex;
			var numQuad = this._numVertex/4;
			this._indices = [];
			for ( var i=0; i<numQuad; i++) {
			...
			// this.iBuffer = this.gl.createBuffer();
			this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, this.iBuffer);
			this.gl.bufferSubData(this.gl.ELEMENT_ARRAY_BUFFER, 0, new Uint16Array(this._indices));
			this.iBuffer.itemSize = 1;
			this.iBuffer.numItems = this._indices.length;
		}
		...
		p.generateIndexBuffer = function(indices) {
			// console.debug( "Generate Index Buffer" );
